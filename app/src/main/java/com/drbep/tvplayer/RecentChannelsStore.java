...
final class RecentChannelsStore {
    ...
    void mergeIds(List<String> channelIds, Map<String, String> namesById) {
        ...
        for (String rawId : channelIds) {
            ...
            // Replays and recordings are not necessarily in the linear catalog.
            // An ID-only server merge must not erase their locally known title.
            if (name == null || name.trim().isEmpty()) {
                for (RecentChannelItem existing : items) {
                    if (id.equals(existing.channelId)) {
                        name = existing.channelName;
                        break;
                    }
                }
            }
