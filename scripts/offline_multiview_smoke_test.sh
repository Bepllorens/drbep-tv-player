...
TRIM_TEST_LEVEL="${TRIM_TEST_LEVEL:-}"
...
Variables opcionales:
  ...
  TRIM_TEST_LEVEL           HIDDEN o RUNNING_CRITICAL, inyeccion ADB de prueba.
EOF
}
...
cleanup() {
  ...
  if [ -n "$LOG_OUTPUT" ]; then cp "$MULTIVIEW_LOG" "$LOG_OUTPUT"; fi
...
"${ADB[@]}" logcat -c || true
START_PID="$("${ADB[@]}" shell pidof "$APP_PACKAGE" | tr -d '\r')"
"${ADB[@]}" logcat --pid="$START_PID" -v threadtime > "$MULTIVIEW_LOG" &
LOG_READER_PID=$!
press KEYCODE_DPAD_CENTER
...
case "$TRIM_TEST_LEVEL" in
  HIDDEN)
    press KEYCODE_HOME
    "${ADB[@]}" shell am send-trim-memory "$APP_PACKAGE" HIDDEN
    "${ADB[@]}" shell am start -n "$APP_PACKAGE/$APP_ACTIVITY" >/dev/null
    ;;
  RUNNING_CRITICAL)
    "${ADB[@]}" shell am send-trim-memory "$APP_PACKAGE" RUNNING_CRITICAL
    ;;
  "") ;;
  *) echo "TRIM_TEST_LEVEL invalido" >&2; exit 1 ;;
esac
...
if [ "$PID" != "$START_PID" ]; then
  echo "La app se reinicio durante Multipantalla" >&2
  exit 1
fi
kill "$LOG_READER_PID" 2>/dev/null || true
wait "$LOG_READER_PID" 2>/dev/null || true
...
if ! grep -q "multiview opened" "$MULTIVIEW_LOG"; then
  echo "Falta evidencia del ciclo de vida de Multipantalla" >&2
  exit 1
fi
if [ "$TRIM_TEST_LEVEL" = "HIDDEN" ] && ! grep -q "memoryTrim level=20 multiview=true closeForPressure=false" "$MULTIVIEW_LOG"; then
  echo "Falta confirmacion de HIDDEN con Multipantalla abierta" >&2
  exit 1
fi
if grep -q "multiview closed" "$MULTIVIEW_LOG"; then
  echo "Multipantalla se cerro antes de terminar la observacion" >&2
  exit 1
fi
...
# No relanzar el canal por intent: eso ocultaria un retorno roto del mosaico.
"${ADB[@]}" logcat -c || true
press KEYCODE_BACK
sleep "$WAIT_BOOT_SECONDS"
